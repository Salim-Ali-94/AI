...
class ArtificialNeuralNetwork(NN.Module):
	...
	def __init__(self, nodes, functions, system = None, flatten = False, unflatten = False, channel = 1, height = 28):
		...
		for index in range(depth):
			...
			if ((index == depth - 1) & (unflatten == True)): self.network.add_module("transform", NN.Unflatten(1, (channel, height, height)))
...
def partition(characteristics, categories, output, batch, training_percentage = 100, validation_percentage = 0, flag = False):
	...
	elif ((output == 1) & (type(training_y) == torch.FloatTensor)): train_y = torch.FloatTensor(training_y)
...
def learn(trainer, neurons, functions, learning_rate, episodes, cost, propagator, validater = [], horizon = 0, kernel = 0, stride = 0, padding = 0, channel = 1, height = 28, pooling = [], convolutions = [], direction = 1, offset = True, normalizing = [], flatten = False, unflatten = False, show = True):
	...
	elif (neurons[-1] > 1): error = criterion(utility["crossentropy"])
	else: error = criterion(utility["mse"])
	if (neurons[-1] == 1): labels = list(set(Y))
	model.train()
	os.system("cls")
	print(), print("_"*120)
...
def train(trainer, functions, learning_rate, episodes, cost, propagator, noise_width, neurons = [], kernel = [], stride = [], padding = [], channel = 1, height = 28, pooling = [], convolutions = [], direction = [], offset = [], normalizing = [], flatten = [], unflatten = [], length = 0, lamda = 0, show = True):
	...
	print(), print("_"*120)
	try: print(), print(summary(critic, (channel, height, height)))
	...
	for epoch in range(episodes):
		...
		for index, (x, y) in enumerate(trainer):
			...
			if (length == 0):
				...
				if (kernel != []): data_fake = torch.randn(len(x), noise_width, 1, 1).to(device)
...
def validate(model, validater, error, horizon, residual, episodes, labels = []):
	...
	if ((validate.epoch == 0) | (validate.epoch == validate.history + 1)): torch.save(model, "nn.pth")
	...
	with torch.no_grad():
		...
		for index, (x, y) in enumerate(validater):
	...
	validate.accuracy.append(sum(batch_accuracy) / len(batch_accuracy))
	...
	if (validate.epoch == validate.period):
		...
		if ((count == horizon) & (counter >= int(0.8*horizon))): 
			...
			print(), print(f"last discarded model;")
			...
			print(), print(f"last reverted model;")
